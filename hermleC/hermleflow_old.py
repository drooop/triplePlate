...
def doublecheck(describeStr):
    i = 1
    while True:
        a = input(describeStr + 'y/n?')
        if a == 'y':
            i += 1
            print('请再次确认。')
            if i > 2:
...
class local_recieve_orderList(task.Task):
    ...
    def execute(self, filename='default', *args, **kwargs):
        ...
        timemsg = {'orderkey':orderlist['orderkey'],'pName':'机床开始加工','pType':'datetime','pValue':orderlist['starttime'],'pDescription':orderlist['step'],'orderstatus':orderlist['orderstatus']}
        try:
...
class MT_readStatus(task.Task):

    def execute(self, *args, **kwargs):
        sendCMD(funcname='readStatus')

# class RebortStayCheck(task.Task):

#     def execute(self, * args, **kwargs):
#         doublecheck('RebortStay Ready?')

# class PutWorkpiece(task.Task):
...
#         valvemotion('20')
#         valvemotion('11')
#         res = valvemotion('read')
#         if res['shelfStatus'][2] == True and res['shelfStatus'][5] == True:
#             print('OK')
#         else:
#             print('warning')

# class RebortPutWork(task.Task):

#     def execute(self, * args, **kwargs):
#         doublecheck('RebortPutWork Ready?')

# class PutWorkpieceCheck(task.Task):
...
#         valvemotion('20')
#         res = valvemotion('read')
#         if (res['shelfStatus'][4] == True or res['shelfStatus'][5] == True):
#             print('warning')
#         else:
#             print('OK')

class MT_Initialize(task.Task):

    def execute(self, *args, **kwargs):
        sendCMD(funcname=tqfDict['f_writeStatus'], filename='8013')
        time.sleep(3.5)
        sendCMD(funcname=tqfDict['f_writeStatus'], filename='8023')
        time.sleep(3.5)
        sendCMD(funcname=tqfDict['f_writeStatus'], filename='8033')
        time.sleep(3.5)
        try:
            tqlogger(orderlist['orderkey'], '初始化完成', orderlist['step'])
        except Exception:
            pass
        # TQlogger.info('初始化完成')
...
class MT_bindPlate(task.Task):
    ...
    def execute(self):
        ...
        plateStatus = {}
        plateStatus['1'] = status['机床状态']['1'][1:3]
        plateStatus['2'] = status['机床状态']['2'][1:3]
        plateStatus['3'] = status['机床状态']['3'][1:3]
...
class MT_taskOver(task.Task):
    ...
    def execute(self, *args, **kwargs):
        ...
        timemsg = {'orderkey':orderlist['orderkey'],'pName':'机床完成加工','pType':'datetime','pValue':orderlist['completedtime'],'pDescription':orderlist['step'] + '-Hermle01','orderstatus':orderlist['orderstatus']}
