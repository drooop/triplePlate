...
class local_recieve_orderList(task.Task):
    ...
    def execute(self, filename='default', *args, **kwargs):
        ...
        """
        # check mechineName
        if not orderlist.get('mechineName'):
            raise Exception('fail to find assinged mechineName in orderList')
        if orderlist['mechineName'] != localMachineName:
            raise Exception('work piece sent to wrong mechine')

        # check plateNumber
        if not orderlist.get('palteNum'):
            raise Exception('fail to find assinged plateNumber in orderList')
        """
        
        timemsg = {
            'orderkey': orderlist['orderkey'],
            'pValue': orderlist['step'] + '开始加工',
            'orderstatus':orderlist['orderstatus'],
            'data':{
                '机床开始加工':{
                    'pName': '机床开始加工',
                    'pType': 'datetime',
                    'pValue': orderlist['starttime'],
                    'pDescription': orderlist['step']
                }
            }
        }
        try:
